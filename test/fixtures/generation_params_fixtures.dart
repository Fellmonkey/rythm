...
class _CompletionLevel {
  const _CompletionLevel(this.name, this.pct, this.abs, this.streak, this.objectType);
  final String name;
  final double pct;
  final int abs;
  final int streak;
...
const _levels = [
  _CompletionLevel('zero', 0, 0, 0, GardenObjectType.sleepingBulb),
  _CompletionLevel('low', 20, 4, 2, GardenObjectType.moss),
  _CompletionLevel('mid', 55, 15, 5, GardenObjectType.bush),
  _CompletionLevel('threshold', 80, 5, 5, GardenObjectType.tree),
  _CompletionLevel('high', 95, 28, 20, GardenObjectType.tree),
];

// ── Time distributions ─────────────────────────────────────

class _TimeDistribution {
  const _TimeDistribution(this.name, this.morning, this.afternoon, this.evening);
  final String name;
...
const _distributions = [
  _TimeDistribution('morning', 1.0, 0.0, 0.0),
  _TimeDistribution('afternoon', 0.0, 1.0, 0.0),
  _TimeDistribution('evening', 0.0, 0.0, 1.0),
  _TimeDistribution('mixed', 0.4, 0.35, 0.25),
];

// ── Fixture matrix ─────────────────────────────────────────

/// 6 archetypes x 5 levels x 4 distributions = 120 params
